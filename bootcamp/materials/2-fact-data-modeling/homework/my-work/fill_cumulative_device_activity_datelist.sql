...
WITH
    ...
    events_grouped AS (SELECT user_id,
                              device_id,
    ...
    events_explode AS (SELECT e.user_id,
                              e.device_id,
                              e.event_date,
    ...
    events_cumulative AS (SELECT user_id,
                                 device_id,
                                 date,
    ...
    -- deduplicate devices
    devices_deduplicated AS (SELECT device_id,
                                    device_type,
                                    browser_type
                             FROM devices
                             GROUP BY device_id, device_type, browser_type),
    -- join events with devices
    events_cumulative_device AS (SELECT ec.user_id,
                                        ec.device_id,
                                        dd.browser_type,
                                        ec.date,
